...
public class ArticleServiceImpl implements IArticleService {
    ...
    public List<String> getAllExistsCategories() {
        return articleMapper.getAllExistsCategories();
    }

    @Override
    public Map<String, Integer> getAllTagsAndCount() {
        List<String> allTagsStr = articleMapper.getAllTags();
        Map<String, Integer> allTagsAndCount = new HashMap<>();
        for(String s : allTagsStr) {
            String[] tags = s.split(",");
            for(String t : tags) {
                allTagsAndCount.put(t, allTagsAndCount.getOrDefault(t, 0) + 1);
            }
        }
        return allTagsAndCount;
    }

    @Override
    public Map<String, Integer> getAllCategoriesAndCount() {
        List<Map<String, Object>> listMap = articleMapper.getAllCategoriesAndCount();
        Map<String, Integer> res = new HashMap<>();
        listMap.forEach((m) -> {
            res.put(m.get("categories") + "", Integer.parseInt(m.get("count") + ""));
        });
        return res;
    }
}